...
pub fn parse_ddl_content(content: &str) -> std::result::Result<(Vec<TableDef>, Vec<String>), DdlError> {
    ...
    // 按分号分割语句
    let statements: Vec<&str> = processed_content
        .split(';')
        .map(|s| s.trim())
        .filter(|s| !s.is_empty())
        .collect();
    ...
    for (i, statement) in statements.iter().enumerate() {
            ...
            && (words[1].eq_ignore_ascii_case("TABLE") || words[1].eq_ignore_ascii_case("table"))
        {
            ...
            // 查找左括号，用于分离表名和列定义
            let left_paren = table_line.find('(').ok_or(DdlError::Parsing(
                "Invalid CREATE TABLE syntax: missing '('".to_string(),
            ))?;
...
fn parse_index_def(line: &str) -> std::result::Result<DdlIndex, DdlError> {
    ...
    // 提取列名，处理括号
    let columns_part = line.split('(').nth(1).ok_or(DdlError::Parsing(format!(
        "Invalid CREATE INDEX syntax: {}",
        line
    )))?;
...
fn parse_column_def(line: &str) -> std::result::Result<DdlColumn, DdlError> {
    ...
    let first_space = line
        ...
        .ok_or(DdlError::Parsing(format!(
            "Invalid column definition: missing data type for '{}'",
            line
        )))?;
    ...
    if let Some(left_paren) = remaining.find('(') {
        // 查找匹配的右括号
        let right_paren = remaining[left_paren..]
            .find(')')
            .ok_or(DdlError::Parsing(format!(
                "Invalid column definition: missing closing parenthesis in data type: {}",
                line
            )))?
            + left_paren
            + 1;
...
fn parse_data_type(typ: &str) -> std::result::Result<(DataType, usize), DdlError> {
    ...
    if typ_lower.starts_with("varchar") {
        ...
        let left_paren = typ_lower.find('(').ok_or(DdlError::Parsing(format!(
            "Invalid VARCHAR syntax: missing size in '{}'",
            typ
        )))?;
        let right_paren = typ_lower[left_paren..]
            .find(')')
            .ok_or(DdlError::Parsing(format!(
                "Invalid VARCHAR syntax: missing closing parenthesis in '{}'",
                typ
            )))?
            + left_paren
            + 1;
    ...
    fn test_create_table_with_auto_increment() {
        ...
        match parse_ddl_content(create_table_sql) {
            Ok(tables) => {
                ...
                assert_eq!(
                    fields[0].name, "id",
                    "Expected field name 'id', got '{}'",
                    fields[0].name
                );
                assert_eq!(
                    fields[0].size, 4,
                    "Expected size 4 for INT, got {}",
                    fields[0].size
                );
                assert_eq!(
                    fields[0].primary_key, true,
                    "Expected id to be primary key, got false"
                );
                assert_eq!(
                    fields[0].auto_increment, true,
                    "Expected id to be AUTO_INCREMENT, got false"
                );

                // Check device_id field (VARCHAR(50))
                assert_eq!(
                    fields[1].name, "device_id",
                    "Expected field name 'device_id', got '{}'",
                    fields[1].name
                );
                assert_eq!(
                    fields[1].size, 50,
                    "Expected size 50 for VARCHAR(50), got {}",
                    fields[1].size
                );
