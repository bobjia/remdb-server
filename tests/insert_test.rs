use remdb::{RemDb, RemDbError};
use std::fs;

fn main() {
    // Initialize platform
    let platform = crate::platform::WindowsPlatform;
    remdb::platform::init_platform(&platform);
    
    // Create database instance
    let mut db = RemDb::new(1024 * 1024 * 100, false, None, None).expect("Failed to create database");
    
    // Read test SQL file
    let sql_content = fs::read_to_string("test_insert.sql").expect("Failed to read test file");
    
    // Split SQL statements (simple splitting by ';')
    ...
    for (i, stmt) in statements.iter().enumerate() {
        println!("Executing statement {}/{}: {}", i + 1, statements.len(), stmt);
        match db.sql_query(stmt) {
            Ok(result) => {
                println!("✓ Success!");
...
// Copy of the platform implementation from main.rs
mod platform {
    use core::ptr;
